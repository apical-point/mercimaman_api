...
class PointService extends Bases\BaseService
{
    // リポジトリ
    protected $pointRepo;

    public function __construct(
        PointRepositoryInterface $pointRepo
    ) {
        // リポジトリ
    ...
    {
        return $this->pointRepo->createItem($data);
    }

    // 複数取得
    public function getList($search=[])
    {
        return $this->pointRepo->getList($search);
    }

    // 複数取得
    public function getItems($where=[], $take=0, $orderByRaw='')
    {
        return $this->pointRepo->getItems($where, $take, $orderByRaw);
    }

    // 1件取得
    public function getItem($where)
    {
        return $this->pointRepo->getItem($where);
    }
    ...
    {
        Log::debug("aaaa");
        return $this->pointRepo->updateItem($where, $data);
    }

    // 1件の削除
    public function deleteItem($where)
    {
        return $this->pointRepo->deleteItem($where);
    }

    // 複数の削除
    public function deleteItems($where)
    {
        return $this->pointRepo->deleteItems($where);
    }
    ...
    // ポイントの合計を取得
    public function getUsersSum($userId)
    {
        return  $this->pointRepo->getUsersSum($userId);
    }
