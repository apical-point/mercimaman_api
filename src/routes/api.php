...
Route::group(['prefix'=>'v1'], function () {
    ...
    // NGワード
    Route::middleware('multiauth:admin')->resource('admin/ngWords', 'Api\NgWordsController'); // NGワード

    // ユーザー
    Route::middleware('multiauth:admin')->post('admin/users/pushMailMagazin', 'Api\UserController@pushMailMagazin'); // ユーザー
    Route::middleware('multiauth:admin')->resource('admin/users', 'Api\UserController');
    //Route::middleware('multiauth:admin')->get('admin/users', 'Api\UserController@index'); // ユーザー
    //Route::middleware('multiauth:admin')->get('admin/users/{id}', 'Api\UserController@show'); // ユーザー詳細
    //Route::middleware('multiauth:admin')->post('admin/users/{id}', 'Api\UserController@update'); // ユーザー詳細

    Route::middleware('multiauth:admin')->delete('admin/auth/userwithdrawal', 'Api\AuthController@withdrawal'); // 退会

    // 商品
    Route::middleware('multiauth:admin')->post('admin/deleteReview', 'Api\ReviewProductController@deleteReview'); // 口コミ削除
    Route::middleware('multiauth:admin')->get('admin/getReviewProduct', 'Api\ReviewProductController@getReviewProduct'); // 口コミ商品情報の取得
    Route::middleware('multiauth:admin')->resource('admin/reviewProducts', 'Api\ReviewProductController'); // 口コミ商品
    Route::middleware('multiauth:admin')->post('admin/reviewProduct/update', 'Api\ReviewProductController@updateReviewProduct'); // 口コミ商品の更新
    Route::middleware('multiauth:admin')->resource('admin/reviewProductCategory', 'Api\ReviewProductCategoryController'); // 口コミ商品のカテゴリー
    Route::middleware('multiauth:admin')->post('admin/reviewProductCategory/update', 'Api\ReviewProductCategoryController@updateCategory'); // 口コミ商品のカテゴリー更新
    Route::middleware('multiauth:admin')->resource('admin/reviewProductOffers/', 'Api\ReviewProductOffersController');    // 口コミ商品募集
    ...
    //プレスリリース
    Route::middleware('multiauth:admin')->get('admin/pressrelease', 'Api\PressreleaseController@index');      // 一覧表示
    Route::middleware('multiauth:admin')->get('admin/pressrelease/{id}', 'Api\PressreleaseController@show');  // 一件取得
    Route::middleware('multiauth:admin')->post('admin/pressrelease', 'Api\PressreleaseController@store');     // 登録
    Route::middleware('multiauth:admin')->post('admin/pressrelease/update', 'Api\PressreleaseController@update'); // 更新
    Route::middleware('multiauth:admin')->delete('admin/pressrelease/{id}', 'Api\PressreleaseController@destroy'); // 削除
    ...
    Route::middleware('multiauth:admin')->post('admin/content/updateTarot/{id}', 'Api\ContentController@updateTarot'); // クロスワード 更新
    ...
    //体験
     ...
     Route::middleware('multiauth:admin')->delete('admin/board/deleteExp', 'Api\BoardController@deleteExp'); // 体験削除
    ...
    //---  イベントカレンダー ---------
    Route::middleware('multiauth:admin')->post('admin/event', 'Api\EventController@store');//登録
    Route::middleware('multiauth:admin')->post('admin/event/{id}', 'Api\EventController@update');//編集
    Route::middleware('multiauth:admin')->get('admin/event/{id}', 'Api\EventController@show');//1件取得
    Route::middleware('multiauth:admin')->delete('admin/event/{id}', 'Api\EventController@destroy');//削除
    Route::middleware('multiauth:admin')->get('admin/event', 'Api\EventController@index');//検索取得
    ...
    //---  成長カレンダー ---------
    Route::middleware('multiauth:admin')->post('admin/growth', 'Api\GrowthController@store');//登録
    Route::middleware('multiauth:admin')->post('admin/growth/{id}', 'Api\GrowthController@update');//編集
    Route::middleware('multiauth:admin')->get('admin/growth/{id}', 'Api\GrowthController@show');//1件取得
    Route::middleware('multiauth:admin')->delete('admin/growth/{id}', 'Api\GrowthController@destroy');//削除
    Route::middleware('multiauth:admin')->get('admin/growth', 'Api\GrowthController@index');//検索取得
    //Route::middleware('multiauth:admin')->resource('admin/growth', 'Api\GrowthController');


    //---  ユーザー作成イベントカレンダー ---------userEventMemberUpdate
    Route::middleware('multiauth:admin')->post('admin/userEvent/userEventMemberUpdate/{id}', 'Api\UserEventController@userEventMemberUpdate');//更新
    Route::middleware('multiauth:admin')->get('admin/userEvent/getEventMemberList', 'Api\UserEventController@getEventMemberList');
    Route::middleware('multiauth:admin')->put('admin/userEvent/updateStatus', 'Api\UserEventController@updateStatus');
    Route::middleware('multiauth:admin')->get('admin/userEvent/getEventSales', 'Api\UserEventController@getEventSales');

    //Route::middleware('multiauth:admin')->resource('admin/userEvent', 'Api\UserEventController');
    //updateSlider スライダー画像
//    Route::middleware('multiauth:admin')->post('admin/userEvent/updateSlider', 'Api\UserEventController@updateSlider');
//    Route::middleware('multiauth:admin')->get('admin/userEvent/indexSlider', 'Api\UserEventController@indexSlider');



    Route::middleware('multiauth:admin')->post('admin/userEvent', 'Api\UserEventController@store');//登録
    Route::middleware('multiauth:admin')->post('admin/userEvent/{id}', 'Api\UserEventController@update');//編集
    Route::middleware('multiauth:admin')->get('admin/userEvent/{id}', 'Api\UserEventController@show');//1件取得
    Route::middleware('multiauth:admin')->delete('admin/userEvent/{id}', 'Api\UserEventController@destroy');//削除
    Route::middleware('multiauth:admin')->get('admin/userEvent', 'Api\UserEventController@index');//検索取得


    //スライダー画像登録、編集
    Route::middleware('multiauth:admin')->post('admin/topSlideData', 'Api\TopSlideDataController@store');//登録
    Route::middleware('multiauth:admin')->post('admin/topSlideData/{id}', 'Api\TopSlideDataController@update');//編集
    Route::middleware('multiauth:admin')->get('admin/topSlideData/{id}', 'Api\TopSlideDataController@show');//1件取得
    Route::middleware('multiauth:admin')->delete('admin/topSlideData/{id}', 'Api\TopSlideDataController@destroy');//削除
    Route::middleware('multiauth:admin')->get('admin/topSlideData', 'Api\TopSlideDataController@index');//検索取得

    //Route::middleware('multiauth:admin')->resource('admin/topSlideData', 'Api\TopSlideDataController'); // topスライダー画像と遷移先画面情報
    ...
    Route::middleware('multiauth:api')->post('user/setBlock', 'Api\UserController@setBlock'); // ユーザーのブロックを追加・解除
    ...
    Route::middleware('multiauth:api')->post('postReview', 'Api\ReviewProductController@postReview'); // 口コミ投稿
    Route::middleware('multiauth:api')->get('getReviewProduct', 'Api\ReviewProductController@getReviewProduct'); // 口コミ商品情報の取得
    ...
    //--- ユーザー参加イベント
    Route::middleware('multiauth:api')->post('userEvent/userEventStore', 'Api\UserEventController@userEventStore');//会員参加登録と決済
    Route::middleware('multiauth:api')->post('userEvent/userEventMemberUpdate', 'Api\UserEventController@userEventMemberUpdate');//更新
    Route::middleware('multiauth:api')->get('userEvent/getEventMemberList', 'Api\UserEventController@getEventMemberList');
    Route::middleware('multiauth:api')->put('userEvent/updateStatus', 'Api\UserEventController@updateStatus');

    Route::middleware('multiauth:api')->post('userEvent', 'Api\UserEventController@store');//登録
    Route::middleware('multiauth:api')->post('userEvent/{id}', 'Api\UserEventController@update');//編集
    Route::get('userEvent/{id}', 'Api\UserEventController@show');//1件取得
    Route::middleware('multiauth:api')->delete('userEvent/{id}', 'Api\UserEventController@destroy');//削除
